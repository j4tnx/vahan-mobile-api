...
sudo apt install -y python3-pip python3-venv git curl wget unzip libnss3 libnspr4 libatk-bridge2.0-0 libdrm2 libxkbcommon0 libxcomposite1 libxdamage1 libxrandr2 libgbm1 libasound2
...
if [ ! -f "main.py" ]; then
    ...
    echo "📝 Please make sure both main.py and requirements.txt are in ~/vahan-api/"
    exit 1
fi
...
Type=simple
User=$USER
WorkingDirectory=$HOME/vahan-api
Environment=PATH=$HOME/vahan-api/venv/bin
ExecStart=$HOME/vahan-api/venv/bin/gunicorn -w 2 -b 0.0.0.0:5000 main:app
Restart=always
RestartSec=10
